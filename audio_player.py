    ...
    def run(self):
        try:
            with sf.SoundFile(self.file_path, 'r') as audio_file:
                ...
                with sd.OutputStream(samplerate=samplerate, channels=audio_file.channels, dtype='float32') as stream:
...
class MediaPlayer(QWidget):
    ...
    def __init__(self):
        ...
        # 连接滑块拖动事件
        self.slider.sliderMoved.connect(self._seek_to_position)
    ...
    def _init_ui(self):
        ...
        self.info_label.setStyleSheet("font-size: 14px; font-weight: bold; color: #555; margin: 20px;")
    ...
    def _playback_finished(self):
        self.is_playing = False
        self.btn_play.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_MediaPlay))
        self.slider.setValue(self.slider.maximum())  # 确保滑块到终点
